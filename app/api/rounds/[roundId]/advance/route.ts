...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  if (userError || !userData.user) {
    console.error("Game advance authentication failed", {
      roundId,
  ...
  if (error) {
    console.error("Game advance failed", { roundId, code: error.code, message: error.message });
    return NextResponse.json({ error: "Could not advance the game." }, { status: 500 });
  }
